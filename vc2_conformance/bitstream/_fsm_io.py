...
A plain Python 3-tuple may be used in place of this namedtuple type to avoid
the small performance overhead creating instances of this type.
"""
...
class TokenTypes(Enum):
    ...
    nbytes = 1
    """
    The next value will be a fixed-length :py:class:`bytes` string containing
    a whole number of bytes from the bitstream (alignment optional).
    
    Takes a length (in bytes) as argument.
    
    Consumes/produces :py:class:`bytes`.
    """
...
def read_fsm(reader, token_generator, values):
    ...
    token_generator : generator
        A generator which yields :py:class:`Token` tuples (or generic 3-tuples)
        defining what values are to be read from the bitstream. The generator
        will be sent the read values in return.
    ...
    try:
        ...
        latest_value = None
        while True:
            (token_type, token_argument, token_target) = token_generator.send(latest_value)
            if token_type is TokenTypes.nbits:
                latest_value, this_bits_past_eof = read_bits(reader, token_argument)
                bits_past_eof += this_bits_past_eof
            elif token_type is TokenTypes.nbytes:
                latest_value, this_bits_past_eof = read_bytes(reader, token_argument)
                bits_past_eof += this_bits_past_eof
                ...
                reader = BoundedReader(reader, token_argument)
                ...
                raise ValueError("Unrecognised TokenType: {}".format(token_type))
            
            if token_target is not None:
                values[token_target][next_value_index[token_target]] = latest_value
                next_value_index[token_target] += 1
...
def write_fsm(writer, token_generator, values):
    ...
    token_generator : generator
        A generator which yields :py:class:`Token` tuples (or generic 3-tuples)
        defining what values are to be written to the bitstream. The generator
        will be sent the written values in return.
    values : {target_identifier: [value, ...], ...}
        A dictionary which, for each of the target identifiers generated by the
    ...
    try:
        ...
        while True:
            ...
            if token_target is not None:
                latest_value = values[token_target][next_value_index[token_target]]
                next_value_index[token_target] += 1
            else:
                latest_value = 0
            
            if token_type is TokenTypes.nbits:
                if latest_value < 0 or latest_value.bit_length() > token_argument:
                    raise ValueError("Value {} out of range for {}-bit nbits.".format(
                        latest_value, token_argument))
                bits_past_eof += write_bits(writer, token_argument, latest_value)
            elif token_type is TokenTypes.nbytes:
                bits_past_eof += write_bytes(writer, token_argument, latest_value)
            elif token_type is TokenTypes.uint:
                if latest_value < 0:
                ...
                writer = BoundedWriter(writer, token_argument)
...
def fsm_target_at_offset(token_generator, values, target_offset):
    ...
    token_generator : generator
        A generator which yields :py:class:`Token` tuples (or generic 3-tuples)
        defining the bitstream structure. The generator will be sent the values
        encoded by the bitstream in return.
    values : {target_identifier: [value, ...], ...}
        A dictionary which, for each of the target identifiers generated by the
    ...
    try:
        ...
        while True:
            ...
            if token_type is TokenTypes.nbits:
                current_token_length = token_argument
            elif token_type is TokenTypes.nbytes:
                current_token_length = token_argument * 8
            elif token_type is TokenTypes.uint:
                current_token_length = exp_golomb_length(latest_value)
            elif token_type is TokenTypes.sint:
                current_token_length = signed_exp_golomb_length(latest_value)
            elif token_type is TokenTypes.bounded_block_begin:
                bounded_block_remaining_bits.append(token_argument)
            elif token_type is TokenTypes.bounded_block_end:
            ...
            if offset > target_offset:
                if token_target is not None:
                    return (token_target, next_value_index[token_target] - 1)
                else:
                    return (None, None)
    except StopIteration:
        pass
