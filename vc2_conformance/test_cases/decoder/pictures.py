...
def generate_dangling_transform_values(block_bits, num_values, dangle_type, magnitude):
    ...
    for num_values_before in reversed(range(min(
        block_bits - length + 1,
        num_values,
    ))):
        try:
    ...
    return (
        values_before +
        [value] +
        [0]*(num_values - len(values_before) - 1)
    )
