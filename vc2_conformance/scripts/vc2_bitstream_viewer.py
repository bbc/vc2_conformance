...
def format_value_line(offset, raw_bits, label, label_filler="", truncated=False):
    ...
    out = []
    out.append("{:0{}d}: {:<{}s}    {}".format(
        offset, OFFSET_DIGITS,
        raw_bits_lines[0], RAW_BITS_PER_LINE,
        label,
    ))
...
def format_omission_line(offset, num_bits):
    ...
    return "{:0{}d}: {:<{}s}    ...".format(
        offset, OFFSET_DIGITS,
        "<{} bits omitted>".format(num_bits), RAW_BITS_PER_LINE,
    )
...
class BitstreamViewer(object):
    ...
    def _print_value(self, offset, raw_bits, target, value):
        ...
        different = False
        for level, (last_path_element, this_path_element) in enumerate(zip_longest(last_path[:-1], this_path[:-1])):
            different |= last_path_element != this_path_element and this_path_element is not None
        ...
        past_end_of_bounded_block = (
            self._reader.bits_remaining is not None and
            self._reader.bits_remaining < 0
        )
    ...
    def __call__(self, serdes, target, value):
        ...
        enable_display = (
            ...
            (
                self._hidden_types is None or
                self._serdes.cur_context.__class__ not in self._hidden_types
            )
            and
            (
                self._shown_types is None or
                self._serdes.cur_context.__class__ in self._shown_types
            )
        )
        ...
        if self._check_parse_info_prefix:
            ...
            if target == "parse_info_prefix" and value != PARSE_INFO_PREFIX:
                raise BitstreamViewer._TerminateError(
                    "invalid parse_info prefix ({})".format(
                        Hex(8)(value)))
...
def parse_args(*args, **kwargs):
    ...
    * hide (list of str): List of VC-2 pseudocode function names whose
    ...
    parser.add_argument(
        "--show-internal-state", "-i", action="store_true", default=False,
        help="""
    ...
    parser.add_argument(
        "--ignore-parse-info-prefix", "-p", action="store_true", default=False,
        help="""
    ...
    parser.add_argument(
        "--num-trailing-bits", "-b", type=int, default=DEFAULT_CONTEXT_BITS,
        help="""
    ...
    range_group.add_argument(
        "--from-offset", "-f", type=int, metavar="BIT_OFFSET",
        help="""
    ...
    range_group.add_argument(
        "--offset", "-o", metavar="BIT_OFFSET", type=int,
        help="""
            ...
            shown.
        """.format(DEFAULT_CONTEXT_BITS),
    )
    
    range_group.add_argument(
        "--after-context", "-A", metavar="NUM_BITS", type=int,
        help="""
            Sets the number of bits after '--around-offset' to be shown.
        """,
    )
    range_group.add_argument(
        "--before-context", "-B", metavar="NUM_BITS", type=int,
        help="""
            Sets the number of bits before '--around-offset' to be shown.
        """,
    )
    range_group.add_argument(
        "--context", "-C", metavar="NUM_BITS", type=int,
        help="""
    ...
    filter_group.add_argument(
        "--show", "-s", type=str, metavar="FUNCTION", default=[],
        action="append",
        help="""
        ...
        """.format(", ".join(sorted(
            bitstream.pseudocode_function_to_fixeddicts_recursive
        ))),
    )
    
    filter_group.add_argument(
        "--hide", "-H", type=str, metavar="FUNCTION", default=[],
        action="append",
        help="""
    ...
    filter_group.add_argument(
        "--hide-slice", "-S",
        action="append_const", const="slice", dest="hide",
        help="""
    ...
    if args.offset is not None:
        ...
        if args.context is not None and (args.after_context is not None or args.before_context is not None):
            parser.error("--context may not be used at the same time as --after-context or --before-context")
    ...
    for name in args.show:
        if name not in bitstream.pseudocode_function_to_fixeddicts_recursive:
            parser.error("--show includes unrecognised pseudocode function {!r}".format(name))
    for name in args.hide:
        if name not in bitstream.pseudocode_function_to_fixeddicts_recursive:
            parser.error("--hide includes unrecognised pseudocode function {!r}".format(name))
