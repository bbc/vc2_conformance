...
def noise_picture():
    ...
    def randrange(max_value_plus_one, size):
        h, w = size
        return [
            [rand.randrange(max_value_plus_one) for _ in range(w)] for _ in range(h)
        ]
...
def test_read_picture_and_read_and_write_metadata(
    noise_picture, video_parameters, picture_coding_mode
):
    picture_file = BytesIO()
    ...
    assert {k: type(v) for k, v in video_parameters.items()} == {
...
def test_read_and_write_convenience_functions(
    picture, video_parameters, picture_coding_mode, tmp_path
):
