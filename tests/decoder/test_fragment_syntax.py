...
class TestFragmentHeader(object):
    @pytest.mark.parametrize(
        "fragment_slice_count,fragment_slices_remaining,exp_fail",
        [
    ...
    def test_picture_not_allowed_to_change_between_fragments(self):
        ...
        decoder.fragment_header(state)
        with pytest.raises(
            decoder.PictureNumberChangedMidFragmentedPicture
        ) as exc_info:
            decoder.fragment_header(state)
    ...
    def test_must_have_contiguous_slices(
        ...
        state["fragment_slices_received"] = (
            expected_fragment_y_offset * 10 + expected_fragment_x_offset
        )
        state["_last_picture_number"] = 0
        state["slices_x"] = 10
        state["slices_y"] = 10
        ...
        if exp_fail:
            ...
            assert (
                exc_info.value.expected_fragment_x_offset == expected_fragment_x_offset
            )
            assert (
                exc_info.value.expected_fragment_y_offset == expected_fragment_y_offset
            )
        else:
            decoder.fragment_header(state)
