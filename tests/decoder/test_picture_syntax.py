...
class TestQuantisationMatrix(object):
    ...
    @pytest.mark.parametrize(
        "dwt_depth,dwt_depth_ho,matrix,exp_fail_value",
        [
            # No transform
            (0, 0, [0], None),
            (0, 0, [127], None),
    ...
    def test_value_ranges_restricted(
        self, dwt_depth, dwt_depth_ho, matrix, exp_fail_value
    ):
        state = {
            "wavelet_index": tables.WaveletFilters.haar_no_shift,
            "wavelet_index_ho": tables.WaveletFilters.haar_no_shift,
        ...
        if exp_fail_value is not None:
            with pytest.raises(
                decoder.QuantisationMatrixValueNotAllowedInLevel
            ) as exc_info:
                decoder.quant_matrix(state)
