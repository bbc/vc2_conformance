...
class TestParseRegex(object):
    ...
    def test_multiple_modifiers(self):
        with pytest.raises(
            SymbolRegexSyntaxError, match=r"Multiple modifiers at position 4"
        ):
            parse_regex("foo ? *")

    def test_modifier_before_union(self):
        with pytest.raises(
            SymbolRegexSyntaxError, match=r"Modifier before '\|' at position 2"
        ):
            parse_regex("a | ? b")

    def test_modifier_before_parenthesis(self):
        with pytest.raises(
            SymbolRegexSyntaxError, match=r"Modifier before '\(' at position 1"
        ):
            parse_regex(" ( ? foo ) ")

    def test_modifier_at_start_of_expression(self):
        with pytest.raises(
            SymbolRegexSyntaxError, match=r"Modifier at start of expression"
        ):
            parse_regex(" ? foo ")

    def test_unmatched_closing_parentheses(self):
        with pytest.raises(SymbolRegexSyntaxError, match=r"Unmatched parentheses"):
            parse_regex(" ( foo ) )")
