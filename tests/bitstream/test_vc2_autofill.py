...
class TestAutofillParseOffsets(object):
    ...
    def test_padding_and_aux_data(self, parse_code):
        seq = Sequence(
            data_units=[
                ...
                DataUnit(
                    parse_info=ParseInfo(parse_code=parse_code, next_parse_offset=100,),
                ),
            ]
        )
        for data_unit in seq["data_units"]:
    ...
    def test_finalizer_works(self):
        ...
        parse_infos = [
            data_unit["parse_info"] for data_unit in serdes.context["data_units"]
        ]
            ...
            DataUnit(
                ...
                sequence_header=SequenceHeader(
                    video_parameters=SourceParameters(
                        ...
                        frame_size=FrameSize(
                            custom_dimensions_flag=True, frame_width=4, frame_height=4,
                        )
                    ),
                ),
            ),
