...
@pytest.mark.parametrize("string,expected,exception", [
    # Plain integers
    ("0", (False, 0), None),
    ("123", (False, 123), None),
    ("-123", (False, -123), None),
...
class TestMostRecentPseudocodeFunction(object):
    ...
    def test_function_in_bitstream_module(self):
        try:
            ...
            exc_type, exc_value, exc_tb = sys.exc_info()
        assert most_recent_pseudocode_function(exc_tb) == "transform_parameters (12.4.1)"
...
class TestBitstreamViewer(object):
    ...
    @pytest.mark.parametrize("shown,hidden,exp_shown,exp_hidden", [
        # Empty case
        ([], [], None, None),
    ...
    def test_check_parse_info_prefix(self, capsys, bad_parse_info_prefix_bitstream_fname):
        # Checking enabled: should stop on bad prefix
        v = BitstreamViewer(
            bad_parse_info_prefix_bitstream_fname,
...
class TestParseArgs(object):
    ...
    @pytest.mark.parametrize("args,exp_from,exp_to", [
        # Default: Whole file
        ("", 0, -1),
    ...
    @pytest.mark.parametrize("args,message_pattern", [
        ...
        ("foo -o 100 -C 10 -A 10", r".*--context may not be used at the same time as.*"),
        ("foo -o 100 -C 10 -B 10", r".*--context may not be used at the same time as.*"),
        # Invalid show/hide arguments
        ("foo -s foobar", r".*--show includes unrecognised pseudocode function 'foobar'.*"),
        ("foo -H foobar", r".*--hide includes unrecognised pseudocode function 'foobar'.*"),
    ])
