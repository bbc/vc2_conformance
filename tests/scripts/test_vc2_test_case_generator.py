...
class TestLoadCodecFeatures(object):
    ...
    @pytest.mark.parametrize("regex,exp_names", [
...
@pytest.mark.parametrize("exp_fail,files,check_encoder,check_decoder", [
    # No files
    (False, [], True, True),
    (False, [], False, True),
    (False, [], True, False),
    ...
    with alternative_real_pictures():
        for name, codec_features in codec_feature_sets.items():
            for test_case in ENCODER_TEST_CASE_GENERATOR_REGISTRY.generate_test_cases(
                ...
                encoder_test_files.add(os.path.join(
                    name,
                    "encoder",
                    test_case.name,
                    "picture_0.raw",
            ...
            for test_case in DECODER_TEST_CASE_GENERATOR_REGISTRY.generate_test_cases(
                ...
                decoder_test_files.add(os.path.join(
                    name,
                    "decoder",
...
@pytest.mark.parametrize("extra_args,exp_fail", [
    ([], True),
    (["--force"], False),
])
def test_force(
...
@pytest.mark.parametrize("extra_args,exp_encoder,exp_decoder", [
    ([], True, True),
