...
from vc2_conformance.test_cases.video_parameters_to_bitstream import (
...
class TestIterCustomOptionsDicts(object):
    ...
    @pytest.mark.parametrize("bvp,vp,exp", [
        # Override required
        (
            VideoParameters(
                frame_width=640,
    ...
    def test_no_presets(self, bvp, vp, exp):
        dicts = list(iter_custom_options_dicts(
            bvp,
            vp,
            dict_type=FrameSize,
...
class TestIterSourceParameterOptions(object):
    ...
    def test_valid_bitstream(self):
        ...
        source_parameters_sets = list(iter_source_parameter_options(
            video_parameters,
            video_parameters,
        ))
    ...
    def test_mismatched_top_field_first(self):
        ...
        assert list(iter_source_parameter_options(
...
class TestCountVideoParameterDifferences(object):
    ...
    def test_empty(self):
        assert count_video_parameter_differences(
            VideoParameters(),
            VideoParameters(),
        ) == 0
